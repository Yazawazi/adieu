...
impl Writeable for Flag {
    fn byte_size(&self) -> usize {
        mem::size_of::<u8>() + self.unk1.byte_size() + self.flags.byte_size()
    }
    ...
    fn byte_size(&self) -> usize {
        match self {
            ...
            FormattedTextCmd::IntegerZeroPadded(idx, zeros) => {
                1 + idx.byte_size() + zeros.byte_size()
            }
    ...
    fn byte_size(&self) -> usize {
        match self {
            ...
            FadeCmd::FadeTimedColor(r, g, b, fadestep) => {
                1 + r.byte_size() + g.byte_size() + b.byte_size() + fadestep.byte_size()
            }
...
impl Writeable for GrpCompositeChild {
    fn byte_size(&self) -> usize {
        ...
        1 + self.file.byte_size() + method_size
    }
    ...
    fn byte_size(&self) -> usize {
        match self {
            ...
            SndCmd::MoviePlay(a, b, c, d, e) => {
                1 + a.byte_size() + b.byte_size() + c.byte_size() + d.byte_size() + e.byte_size()
            }
    ...
    fn byte_size(&self) -> usize {
        match self {
            ChoiceCmd::Choice(idx, flag, texts) => {
                1 + idx.byte_size() + flag.byte_size() + texts.byte_size()
            }
    ...
    fn byte_size(&self) -> usize {
        match self {
            ...
            StringCmd::Strcmp(dest, text1, text2) => {
                1 + dest.byte_size() + text1.byte_size() + text2.byte_size()
            }
            ...
            StringCmd::Itoa(dest, src, ordinal) => {
                1 + dest.byte_size() + src.byte_size() + ordinal.byte_size()
            }
    ...
    fn byte_size(&self) -> usize {
        match self {
            SetMultiCmd::Val(start_idx, end_idx, value) => {
                1 + start_idx.byte_size() + end_idx.byte_size() + value.byte_size()
            }
...
impl Writeable for BGCopyWholeScreen {
    fn byte_size(&self) -> usize {
        self.srcpdt.byte_size() + self.dstpdt.byte_size() + self.flag.byte_size()
    }
    ...
    fn byte_size(&self) -> usize {
        match self {
            FlashGrpCmd::FillColor(dstpdt, r, g, b) => {
                1 + dstpdt.byte_size() + r.byte_size() + g.byte_size() + b.byte_size()
            }
    ...
    fn byte_size(&self) -> usize {
        match self {
            MultiPdtCmd::Slideshow(pos, wait, entries) => {
                1 + mem::size_of::<u8>() + pos.byte_size() + wait.byte_size() + entries.byte_size()
            }
    ...
    fn byte_size(&self) -> usize {
        match self {
            ...
            AreaBufferCmd::GetArea(x, y, area) => {
                1 + x.byte_size() + y.byte_size() + area.byte_size()
            }
            AreaBufferCmd::AssignArea(area_from, area_to) => {
                1 + area_from.byte_size() + area_to.byte_size()
            }
        }
    }
    ...
    fn byte_size(&self) -> usize {
        match self {
            WindowVarCmd::GetBgFlagColor(attr, r, g, b) => {
                1 + attr.byte_size() + r.byte_size() + g.byte_size() + b.byte_size()
            }
    ...
    fn byte_size(&self) -> usize {
        match self {
            ...
            PopupMenuCmd::GetItemDisabled(item_idx, val) => {
                1 + item_idx.byte_size() + val.byte_size()
            }
